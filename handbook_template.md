...
| 分数  | 面试漏斗信号 | 典型特征 |
|-------|-------------|----------|
| 2–3 分 | 几乎为零     | 内容稀薄；过不了 AI 初筛；看不出做过什么、要找什么方向 |
| 5–6 分 | 很低         | 内容齐全但没有量化数据；看不出复杂度和影响力 |
| 7–8 分 | 一般         | 量化清晰、能力可见；但和同类候选人差异不明显 |
| 9–10 分 | 较强        | 在 7–8 分基础上具备**差异化人设**——HR 在 30 秒内就能记住你是谁 |

这些是定性的教学启发式，不是测量结果——只作为相对校准用，不是预测保证。
